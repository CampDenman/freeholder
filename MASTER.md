...
6. **First-party analytics.** Privacy-first pageview + event capture, stored locally, joined to the spine. No third-party pixels in core.
...
Embeddings for retrieval use pgvector — inside the one sacred database (principle 11, §2), not a bolt-on vector store.
