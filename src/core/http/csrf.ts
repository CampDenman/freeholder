...
export function requireCsrf(request: Request): void {
  ...
  // Bearer-only clients send no ambient credential and need no CSRF token.
  // If a session cookie is present, require CSRF even alongside a header:
  // cookie-reading adapters such as logout still use that cookie (C10.25).
