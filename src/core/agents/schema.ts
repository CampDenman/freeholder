...
// Runs, steps, approvals and spend used to live here. They moved to
// `core/runs` in C9.02 (§4.17): an automation that mixes a prompt step with a
// deterministic one must produce one inspectable run, and a run whose owner is
// a NOT NULL `task_id` could never belong to an automation. The agent layer
// still owns agents, tasks, playbooks and connections — the things that are
