...
const imageUrlField = z
  ...
  .superRefine((value, ctx) => {
    ...
    try {
      ...
      if (internalPath(value) !== null) return;
    }
    ctx.addIssue({
      code: "custom",
      message: "Use an on-site image path or an http(s) image URL.",
    });
  });
...
export const shareVia = defineService({
  ...
  rateLimit: {
    limit: 120,
    windowSeconds: 15 * 60,
    // A path-based bucket is bypassed by changing the path and lets one caller
    // grow the table without a ceiling. Prefer a signed-in identity, then the
    // deployment-validated client address, with one bounded fallback bucket.
    subject: (_input, actor) =>
...
const ownerTargetRow = row({
  ...
  locale: z.string(),
  shareable: z.boolean(),
  channels: listed(z.string()).nullable(),
  socialTitle: z.string().nullable(),
  socialDescription: z.string().nullable(),
  imageUrl: z.string().nullable(),
  shares: z.number().int(),
...
export const saveTarget = defineService({
  ...
  handler: async (input, ctx) => {
    ...
    const values = {
      ...
      // Every channel ticked means the null/default "all" state. An empty
      // array remains distinct and means that this entity offers no channels.
      channels:
        input.channels.length === SHARE_CHANNELS.length ? null : [...input.channels],
