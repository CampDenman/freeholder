...
async function configurationRecord(configPath, environment) {
...
export async function createOwnershipExport({
  ...
  try {
    ...
    const configRelative = path.join("configuration", path.basename(configPath));
    await mkdir(path.join(output, "configuration"), { recursive: true, mode: 0o700 });
    await copyFile(
      configPath,
      path.join(output, configRelative),
      fsConstants.COPYFILE_EXCL,
    );
    await chmod(path.join(output, configRelative), 0o600);
    files.push({
      path: configRelative.replaceAll(path.sep, "/"),
      bytes: Buffer.byteLength(configSource),
      sha256: sha256(configSource),
    });

    const recovery = await configurationRecord(configPath, environment);
