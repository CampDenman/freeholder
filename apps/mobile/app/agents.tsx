...
export default function Agents() {
  ...
  const agents = useScreenData<Agent[]>({ screen: "agents", service: "agents.list", caller, cache: memoryCache, enabled: Boolean(session && staff) });
  const board = useScreenData<Board>({ screen: "agents", service: "agents.board", caller, cache: memoryCache, enabled: Boolean(session && staff) });
  ...
  return <StaffScreen><Screen brand={brand}>
    ...
    {agents.loading || board.loading ? <Loading brand={brand} /> : agents.error || board.error ? <Problem brand={brand} message={agents.error ?? board.error!} onRetry={reload} /> : !rows.length ? <Empty brand={brand} message={t(SCREENS.agents.emptyKey)} /> : <ScrollView contentContainerStyle={{ gap: 12, paddingBottom: 24 }}>
