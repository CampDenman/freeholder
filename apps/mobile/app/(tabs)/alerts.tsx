...
export default function Alerts() {
  ...
  const data = useScreenData<Alert[]>({ screen: "alerts", service: "notifications.list", caller, cache: memoryCache, params: { state: "critical", limit: 50 }, enabled: Boolean(session && audience === "staff") });
  ...
  return <StaffScreen><Screen brand={brand}>
    ...
    {data.loading ? <Loading brand={brand} /> : data.error ? <Problem brand={brand} message={data.error} onRetry={reload} /> : !data.value?.length ? <Empty brand={brand} message={t(SCREENS.alerts.emptyKey)} /> : <ScrollView contentContainerStyle={{ gap: 16, paddingBottom: 24 }}>
