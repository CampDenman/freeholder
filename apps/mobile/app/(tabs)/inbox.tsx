...
export default function Inbox() {
  ...
  const data = useScreenData<Thread[]>({ screen: "inbox", service: "conversations.list", caller, cache: memoryCache, params: { limit: 50 }, enabled: Boolean(session && audience === "staff") });
  ...
  return <StaffScreen><Screen brand={brand}>
    ...
    {data.loading ? <Loading brand={brand} /> : data.error ? <Problem brand={brand} message={data.error} onRetry={reload} /> : !data.value?.length ? <Empty brand={brand} message={t(SCREENS.inbox.emptyKey)} /> : <ScrollView>
