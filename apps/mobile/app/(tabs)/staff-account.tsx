...
export default function StaffAccount() {
  const { instance, brand, session, audience, deviceToken, signOut } = useInstance();
  const t = useAppText();
  ...
  const who = useScreenData<Who>({ screen: "staffAccount", service: "auth.whoami", caller, cache: memoryCache, params: { token: session?.token }, enabled: Boolean(session && audience === "staff") });
  ...
  return <StaffScreen><Screen brand={brand}>
    ...
    {who.loading ? <Loading brand={brand} /> : who.error ? <Problem brand={brand} message={who.error} onRetry={reload} /> : !who.value ? <Empty brand={brand} message={t(SCREENS.staffAccount.emptyKey)} /> : <ScrollView contentContainerStyle={{ gap: 12, paddingBottom: 24 }}>
