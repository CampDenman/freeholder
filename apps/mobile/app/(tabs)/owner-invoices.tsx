...
export default function OwnerInvoices() {
  ...
  const data = useScreenData<Invoice[]>({ screen: "ownerInvoices", service: "invoicing.list", caller, cache: memoryCache, params: { limit: 50 }, enabled: Boolean(session) });
  const contacts = useScreenData<Contacts>({ screen: "ownerInvoices", service: "contacts.list", caller, cache: memoryCache, params: { search: search.trim() || undefined, limit: 8 }, enabled: Boolean(session) });
  const create = useScreenWrite<Draft>({ screen: "ownerInvoices", service: "invoicing.createDraft", caller, online });
  const issue = useScreenWrite<{ invoice: { id: string } }>({ screen: "ownerInvoices", service: "invoicing.issue", caller, online });
  ...
  const createDraft = async () => {
    ...
    try {
      const draft = await create.execute({
        ...
        idempotencyKey: `companion:${contactId}:${Date.now()}`,
        lines: [{ description: description.trim(), quantityMicros: 1_000_000, unitAmountMinor: minor }],
        tax: { mode: "not_applicable", reason: t("app.ownerInvoice.taxReason") },
      });
      const issued = await issue.execute({ id: draft.invoice.id });
      setDescription(""); setAmount(""); setContactId(null);
      setNotice(t("app.ownerInvoice.issued"));
      data.reload();
      router.push({ pathname: "/owner-invoice/[id]", params: { id: issued.invoice.id } });
    } catch { /* useScreenWrite records the error */ }
  };
  return <Screen brand={brand}>
    <Title brand={brand}>{t(SCREENS.ownerInvoices.titleKey)}</Title>
    {!session ? <SignIn /> : data.loading ? <Loading brand={brand} /> : data.error ? <Problem brand={brand} message={data.error} onRetry={data.reload} /> : <ScrollView contentContainerStyle={{ gap: 12, paddingBottom: 24 }}>
      ...
      {create.error || issue.error ? <Problem brand={brand} message={create.error ?? issue.error!} /> : null}
