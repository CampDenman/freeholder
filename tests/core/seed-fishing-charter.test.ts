...
import { FORMS, IMAGES, PAGES, TRANSLATIONS, BUSINESS } from "../../seed/fishing-charter/content";
...
describe("the fishing-charter seed content", () => {
  ...
  it("ships booking intake that is not a guaranteed trip", () => {
    const book = FORMS.find((form) => form.slug === "book");
    expect(book).toBeTruthy();
    const keys = new Set(book?.fields.map((field) => field.key));
    expect(keys.has("consent_contact")).toBe(true);
    expect(keys.has("date")).toBe(true);
    expect(keys.has("trip")).toBe(true);
    expect(book?.successMessage.toLowerCase()).toContain("not a guaranteed trip");
  });

  it("contains no live-client brand or New Vibe City fiction", () => {
    for (const phrase of forbidden) {
      expect(seedText).not.toContain(phrase);
    }
    expect(BUSINESS.name).toBe("Your Charter");
    expect(BUSINESS.schemaType).toBe("SportsActivityLocation");
