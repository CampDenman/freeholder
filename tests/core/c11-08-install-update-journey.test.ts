...
describe.runIf(hasDatabase)("C11.08 demo import update rollback", { timeout: 90_000 }, () => {
  ...
  it("commits an empty slug as the CMS home page, not /page", async () => {
    const run = await startImport.call({ origin: "https://example.com", kind: "html" }, OWNER);
    await previewImport.call(
      {
        id: run.id,
        pages: [{ url: "https://example.com/", slug: "", title: "Home", body: "<p>Hello</p>" }],
      },
      OWNER,
    );
    await commitImport.call({ id: run.id }, OWNER);
    const [home] = await db().select().from(pages).where(eq(pages.slug, ""));
    expect(home).toMatchObject({ slug: "", title: "Home", status: "draft" });
    expect(await db().select().from(pages).where(eq(pages.slug, "page"))).toHaveLength(0);
  });

  it("restores a replaced CMS page on import rollback instead of deleting it", async () => {
    const original = await createPage.call(
      {
        slug: "about",
        title: "Original about",
        blocks: [
          {
            id: "original-about-heading",
            type: "heading",
            props: { text: "Original about", level: 1, align: "start" },
          },
        ],
        seo: {},
      },
    ...
    await publishPage.call({ id: original.id, published: true }, OWNER);

    const run = await startImport.call({ origin: "https://example.com", kind: "wordpress-rest" }, OWNER);
    await previewFromSource.call(
      {
        id: run.id,
        payload: JSON.stringify([
          {
    ...
    await reviewImportConflicts.call(
      { id: run.id, conflicts: [{ slug: "About", resolution: "replace" }] },
      OWNER,
    );
    await commitImport.call({ id: run.id }, OWNER);
    const [replaced] = await db().select().from(pages).where(eq(pages.slug, "about"));
    expect(replaced?.id).toBe(original.id);

    await rollbackImport.call({ id: run.id }, OWNER);
    const [restored] = await db().select().from(pages).where(eq(pages.slug, "about"));
