...
| C11.14 leftovers | `search.query` covers registered live sources; other titled contact-attached stores (orders, subscriptions, remaining SEARCH_TABLE_OPT_OUTS) stay per-list. Per-record restore is merge-undo + instance drill, not undelete-every-row. Retention is privacy-rights + artifact TTL, not per-table TTL. |
