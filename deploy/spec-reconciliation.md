...
| C11.14 leftovers | `search.query` covers registered live sources; per-list search remains as specialised lists. Per-record restore is merge-undo + instance drill, not undelete-every-row. Retention is privacy-rights + artifact TTL, not per-table TTL. |
